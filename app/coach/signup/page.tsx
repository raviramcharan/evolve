...
export default function CoachSignupPage() {
  ...
  async function handleGoogle() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=coach-onboarding`,
      },
    })
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-5 py-12">
      <div className="w-full max-w-sm">
        <div className="mb-10 text-center">
          ...
          <p className="font-display text-2xl font-semibold text-text mb-2">Coach sign up</p>
          <p className="text-muted text-sm">Create your coaching account to get started.</p>
        </div>
